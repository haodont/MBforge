...
def _snapshot_result(path: Path, info: ResourceInfo) -> ResourceStatusResult:
    ...
    from mbforge.adapters.runtime import resource_manager as _rm
...
def _search_modelscope(
    ...
    if not ms_root.exists() or not info.ms_repo:
        return None
...
def _check_model_snapshot(info: ResourceInfo | None) -> ResourceStatusResult:
    ...
    # ModelScope-only repos have ``ms_repo`` set; HF-only repos (e.g.
    # Hiro-Layout, SLANet-1M) leave it empty and carry ``hf_repo``. Deriving
    # the repo tail from an empty ``ms_repo`` collapses to the cache root and
    # every snapshot check fails — fall back to hf_repo/id.
    repo_name = (info.ms_repo or info.hf_repo or info.id).split("/")[-1]
    ms_org = info.ms_repo.split("/")[0] if info.ms_repo else ""
