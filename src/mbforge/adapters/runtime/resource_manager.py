...
from mbforge.adapters.runtime.resource_types import (
    EnvironmentReport,
    ResourceInfo,
    ResourceStatus,
...
from mbforge.foundation.files import dir_size_bytes as _helpers_dir_size_bytes
from mbforge.foundation.files import sha256_file as _helpers_sha256_file
...
RESOURCE_CATALOG: dict[str, ResourceInfo] = {
    ...
    "slanet_table": ResourceInfo(
        id="slanet_table",
        name="SLANet-1M Table Structure",
        type=ResourceType.MODEL,
        description=(
            "SLANet-1M 表格结构识别（ONNX 导出，dimtri009 微调，MIT）："
            "输出结构 token 序列 + 每 token 单元格坐标，配合 RapidOCR "
            "还原表格 HTML。仅需 onnxruntime，无 paddlepaddle 依赖。"
        ),
        size_mb=8,
        license="mit",
        license_url="https://huggingface.co/dimtri009/SLANet-1M",
        ms_repo="",
        # ONNX 再导出仓库（源权重为 PaddlePaddle 格式 dimtri009/SLANet-1M）。
        hf_repo="bdatdo0601/slanet-1m-onnx",
        download_type="snapshot",
        local_name="SLANet-1M",
        files=["slanet_1m.onnx", "inference.yml"],
        allow_patterns=["*.onnx", "*.yml"],
        source_url="https://huggingface.co/dimtri009/SLANet-1M",
    ),
    # ──── Python 包（清华源）────
    "rdkit": ResourceInfo(
        id="rdkit",
...
# ``mbforge.adapters.runtime.resource_manager._check_model_*`` / ``_download_model_from_*``
# keep working.


def _load_locator() -> tuple[object, object]:
    from mbforge.adapters.runtime.model_locator import (
        _check_model_file,
...
def _download_model_from_modelscope(
    ...
    from mbforge.adapters.runtime.model_downloader import (
...
def _record_model_outcome(resource_id: str, error: str | None) -> None:
    ...
    :func:`mbforge.adapters.runtime.models.ensure` so download/load outcomes share one
    state channel with prewarm and the model lifecycle endpoints.
    """
    try:
        from mbforge.adapters.runtime.models import ensure as ensure_model_status
...
class ResourceManager:
    ...
    @classmethod
    def get_slanet_path(cls) -> Path | None:
        """获取 SLANet-1M 表格识别模型目录（含 slanet_1m.onnx + inference.yml）."""
        path = cls.get_model_path("slanet_table")
        if path and path.exists():
            return path
        return None
