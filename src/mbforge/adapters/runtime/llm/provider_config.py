...
Consumed by the chat-model factory (:mod:`mbforge.adapters.runtime.llm`), the
model-list probe (:mod:`mbforge.adapters.runtime.llm.provider_models`), the readiness
diagnostics (``application/use_cases/system/readiness.py``) and activity
extraction (``application/pipeline/activity/extraction.py``). Keeping the default base URLs and
