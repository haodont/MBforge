...
Run standalone::
    ...
    python -m mbforge.adapters.inference.ocr.daemon --port 18799 --device-id 0
...
def _get_engine():
    ...
    if _engine is None:
        with _engine_lock:
            if _engine is None:
                from mbforge.adapters.inference.ocr.crop_labels import _create_engine
...
def build_app(device_id: int = 0) -> FastAPI:
    ...
    from mbforge.adapters.inference.ocr.crop_labels import _label_reads_from_result
...
def main() -> None:
    """``python -m mbforge.adapters.inference.ocr.daemon`` entrypoint."""
    import argparse

    import uvicorn

    from mbforge.foundation.logger import configure_uvicorn_access_logging
