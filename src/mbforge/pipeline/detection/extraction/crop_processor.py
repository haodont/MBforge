...
if TYPE_CHECKING:
    from mbforge.infra.process import TaskHandle

logger = get_logger("mbforge.pipeline.detection.extraction.crop_processor")
...
def ocr_label_image(image: Image.Image) -> tuple[list[str], str]:
    ...
    from mbforge.backends.ocr.label_reader import get_label_reader
    from mbforge.pipeline.detection.recognition import select_primary_coref
...
class CropProcessor:
    ...
    def _submit_label_ocr(
        ...
        if main_mask is not None:
            from mbforge.pipeline.detection.image_preprocessing import erase_ink_region
        ...
        from mbforge.infra.process import TaskPool, tasks
