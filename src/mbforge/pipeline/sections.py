...
from ..core.evidence import _stable_id
from .labels import _explicit_compound_labels
from .markdown.markers import _ESMILES_BLOCK_RE
...
def make_section_id(doc_id: str, page_start: int | None, title: str) -> str:
    """Keep section IDs stable across the two stage projections."""
    return _stable_id("section", doc_id, str(page_start or 0), title)
...
def parse_source_evidence_sections(
    ...
    for position, head_index in enumerate(head_indices):
        ...
        sections.append(
            ParsedSection(
                section_id=make_section_id(doc_id, page_start, section_title(heading)),
