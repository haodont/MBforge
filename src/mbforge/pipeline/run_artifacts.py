...
from .artifacts import PatentFactsArtifact
from .run_ids import stage_kind_file
from .stage_artifacts import _branch_path, _read_json

logger = get_logger("mbforge.pipeline.run_artifacts")
...
def reap_stage_run(
    ...
    if kind is None:
        return
    branch_path = _branch_path(library_root, doc_id, kind)
    data = _read_json(branch_path)
