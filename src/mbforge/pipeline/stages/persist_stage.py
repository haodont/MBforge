...
from ..persist.document_publish import (
...
class PersistStage:
    ...
    def _persist_molecules_and_links(self, ctx: PipelineContext) -> None:
        ...
        from ...storage.markush_candidates import persist_review_candidates
        from ...storage.review_audit import insert_review_item
        from ...storage.sqlite.database import DatabaseManager
        from ..detection.structure_role import classify_structure_role
        from ..persist.activities import persist_activities
        from ..persist.markush import (
            delete_markush_for_doc,
            persist_markush_fragments,
            persist_markush_scaffolds,
        )
        from ..persist.molecules import persist_molecule_candidates
        from ..persist.text_links import enrich_molecule_contexts_from_markdown
        ...
        try:
            from ..activity.family_gate import (
                derive_family_core,
        ...
        def _register_links_in_txn(mol_conn: Any) -> None:
            ...
            if not complete_candidates or not mapping_md_path:
                return
            from ..persist.text_links import register_molecules_from_text
    ...
    def _persist_source_evidence(self, ctx: PipelineContext) -> int:
        ...
        from ..stage_artifacts import load_document_evidence
    ...
    def _persist_document(self, ctx: PipelineContext) -> None:
        """Delegate to :mod:`mbforge.pipeline.persist.document_publish`."""
