...
from ..artifacts import PatentFactsArtifact, PatentSectionModel
from ..context import PipelineContext
from ..sections import _HEADING_RE, parse_source_evidence_sections
from ..sections import ParsedSection as DocumentSection
    ...
    *,
    ...
    for match in _COMPOUND_TOKEN_RE.finditer(title):
        ...
        for label in re.split(r"\s*(?:和|and)\s*", labels, flags=re.I):
            ...
            entry = CompoundEntry(
                entry_id=make_entry_id(doc_id, section_id, label_key_for(label_raw)),
                doc_id=doc_id,
                label_raw=label_raw,
...
def extract_assay_method_from_title(
    ...
    *,
    ...
    """
    from ...core.activity import AssayMethod, make_assay_method_id
    ...
    method = AssayMethod(
        assay_method_id=make_assay_method_id(doc_id, section_id),
        doc_id=doc_id,
        target=target,
        endpoint=endpoint,
...
def _associate_facts(
    ...
    from ..activity.normalization import normalize_reference_label
...
class PatentStage:
    ...
    def execute(self, ctx: PipelineContext) -> StageResult:
        ...
        from ..cancellation import default_registry, make_cancel_check
        ...
        try:
            ...
            from ..run_artifacts import publish_run
    ...
    def _extract(self, ctx: PipelineContext, cancel_check) -> ExtractionOutcome:
        ...
        from ..activity.extraction import extract_activity_measurements_from_evidence
        from ..stage_artifacts import load_detections, load_document_evidence
