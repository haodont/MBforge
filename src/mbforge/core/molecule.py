"""Per-molecule entity: the single shared molecule value object.
...
The same object is used before and after persistence.  A pre-persistence
instance may have an empty ``mol_id`` and carry transient detection
provenance; database serialization still keeps the molecule row separate
from the related evidence tables.
"""
...
from .types import DetectionSource


class JsonRecordMixin:
    """Shared JSON/row codec for record entities.

    Provides :meth:`to_json` / :meth:`from_json` / :meth:`from_row` and a
    helper for JSON-encoded columns, so each dataclass only implements its
    own :meth:`from_dict` / :meth:`to_dict` field mapping.
    """

    @classmethod
    def _json_col(cls, data: dict, key: str, default: Any) -> Any:
    ...
    def from_row(cls, row: dict):
        """Construct from a DB row (dict-like)."""
        return cls.from_dict(dict(row))


@dataclass
class Molecule(JsonRecordMixin):
    """A molecule as a typed value object.

    Fields mirror the ``molecules`` SQLite table, with transient detection
    provenance available before persistence. JSON columns are exposed as
    native Python types (``dict`` / ``list``). The binary ``fingerprint``
    field is preserved in-memory but excluded from :meth:`to_dict` /
    :meth:`to_json` (not JSON-friendly and recomputable from
    ``canonical_smiles``).
    """
    ...
    # These fields let extraction, review and persistence share this same
    # entity.  Detection rows remain separate evidence records in SQLite.
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        and recomputable from ``canonical_smiles``); detection rows remain
        separate evidence records and are intentionally not serialized here.
        """
        return {
            "mol_id": self.mol_id,
    ...
    def from_dict(cls, data: dict[str, Any]) -> Molecule:
        ...
        mol = cls(
            ...
            sources=[
                ...
                if source in {"image", "text", "manual"}
            ],
            reject_reason=data.get("reject_reason"),
            properties=cls._json_col(data, "properties", {}),
            labels=cls._json_col(data, "labels", []),
            semantic_tags=cls._json_col(data, "semantic_tags", []),
...
@dataclass
class MarkushScaffold(JsonRecordMixin):
...
@dataclass
class MarkushFragment(JsonRecordMixin):
...
def canonicalize_smiles(smiles: str, *, isomeric: bool = False) -> str | None:
    """Return RDKit-canonical SMILES, or ``None`` when *smiles* does not parse.

    Empty / whitespace-only input and parse failures both return ``None`` so
    callers can branch on "no structural identity" directly.
    """
    ...
    except Exception:  # noqa: BLE001 — parse failure === no identity
        return None
    if mol is None:
        return None
...
def molecule_id(smiles: str) -> str | None:
    """Deterministic structural identity (canonical SMILES) for *smiles*.

    Returns ``None`` when the input is empty or not RDKit-parseable, in which
    case the caller falls back to a random id (no structural identity to dedup
    on). Matches the pipeline convention of using ``canonical_smiles`` as the
    ``mol_id``.
    """
