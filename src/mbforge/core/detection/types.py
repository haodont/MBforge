"""Shared dataclasses for normalized molecule records.

These types are the canonical contract between the pipeline (which builds
the records) and the core services (which persist, correct, and review
them). They live in :mod:`mbforge.core.detection` because they are pure
value objects shared across layers — not the persisted entity (that is
:class:`mbforge.core.entities.molecule.Molecule`).
...
class NormalizedMolecule:
    ...
    Attributes:
        ...
        esmiles: Markush E-SMILES preserved for traceability; ordinary
            candidates carry their Layer-1 value here for compatibility.
        name: Compound name, if any was extracted. When multiple detections
            merge, the name is re-selected deterministically by the pipeline
            normalization step and the decision is recorded under
            ``properties["name_selection"]``.
