"""Markush pattern parsing and coverage checking.

Self-contained coref computation for a Markush scaffold:

- :func:`parse_markush` parses an E-SMILES / R-group SMILES into the core
  scaffold, its explicit attachment sites and free-form R-group definitions.
  RDKit atom-map numbers are the canonical attachment identity (implicit
  ``*`` order is rejected).
- :func:`check_markush_coverage` takes a parsed pattern plus a query molecule
  and decides, per attachment site, whether the query is *within scope*,
  *outside scope* or *unknown*.

These helpers are pure functions: no DB I/O, no Pydantic. The HTTP router
wraps them and converts to Pydantic responses via :func:`parse_response` /
:func:`check_response`.
