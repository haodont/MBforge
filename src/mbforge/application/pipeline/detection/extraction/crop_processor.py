...
from mbforge.application.pipeline.cancellation import CancelCheck, TaskCancelledError
...
logger = get_logger("mbforge.application.pipeline.detection.extraction.crop_processor")
...
def ocr_label_image(image: Image.Image) -> tuple[list[str], str]:
    ...
    from mbforge.application.pipeline.detection.recognition import select_primary_coref
    from mbforge.application.ports import get_runtime

    try:
        reads = get_runtime().ocr_label_reader.get_label_reader().read(image)
...
def fill_ocr_slot(slot: list[Any], handle: TaskHandle) -> None:
    """Store completed OCR data without blocking the MolParser worker."""
    try:
        labels, primary = handle.result()
    except Exception as exc:
...
class CropProcessor:
    ...
    def _submit_label_ocr(
        ...
        runtime = get_runtime()
        handle = runtime.process.tasks.submit(
            runtime.process.TaskPool.OCR,
            self.ocr_reader,
            ocr_image,
            on_done=lambda completed, slot=ocr_slot: self.ocr_slot_filler(
                slot, completed
            ),
        )
        self.ocr_futures.append(handle)
        return ocr_slot
