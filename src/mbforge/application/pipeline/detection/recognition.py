...
1. :func:`~mbforge.application.pipeline.detection.image_preprocessing.preprocess_mol_image` splits
...
class _MolParserPort:
    """Lazy runtime seam retained for direct recognition callers and tests."""

    @staticmethod
    def predict(image: Image.Image):
        return get_runtime().molparser.predict(image)


molparser = _MolParserPort()


def extract_label_reads(image: Image.Image):
    """Read compound labels through the configured runtime provider."""
    return get_runtime().ocr_crop_labels.extract_label_reads(image)
