...
from mbforge.application.pipeline.run.checkpoint import collect_all_summaries
...
class Finalizer:
    ...
    def cleanup_on_failure(self) -> None:
        ...
        if not any(item.get("status") == "success" for item in summaries.values()):
            from mbforge.application.pipeline.artifacts.staging import cleanup_staging
