...
Queue reads/updates live in :mod:`mbforge.adapters.persistence.markush_transitions`;
the shared state machine in :mod:`mbforge.domain.review`; the initial
queue persistence in :mod:`mbforge.adapters.persistence.markush_candidates`.
...
from mbforge.application.ports.repositories import ReviewRepository
...
logger = get_logger("mbforge.application.use_cases.markush.review")


def _fetch_one_compat(
    conn: sqlite3.Connection, sql: str, params: tuple[Any, ...]
) -> sqlite3.Row | None:
    """Keep the connection-level API usable by existing callers."""
    return conn.execute(sql, params).fetchone()


def _record_review_decision_compat(
...
def _copy_evidence_compat(
    ...
    rows = conn.execute(
        "SELECT * FROM markush_evidence WHERE entity_id = ? "
        "AND entity_type = 'review_candidate'",
        (row["candidate_id"],),
    ).fetchall()
    for evidence in rows:
        conn.execute(
            ...
            (
                ...
                evidence["bbox_x1"],
                evidence["bbox_y1"],
                evidence["crop_relpath"],
                evidence["context_text"],
                evidence["moldet_confidence"],
                evidence["scribe_confidence"],
                evidence["composite_confidence"],
            ),
        )
...
def apply_decision(
    ...
    *,
    ...
    new_state, payload = _transition(conn, row, action, review_repository)
    conn.execute(
        """
...
def _transition(
    ...
    if action == "confirm_complete":
        ...
        mol_id = _confirm_complete_to_molecules(conn, row, review_repository)
    ...
    if action == "confirm_scaffold":
        ...
        scaffold_id = _confirm_scaffold_to_markush_scaffolds(
            conn, row, review_repository
        )
    ...
    if action == "confirm_fragment":
        ...
        fragment_id = _confirm_fragment_to_markush_fragments(
            conn, row, review_repository
        )
...
def _confirm_complete_to_molecules(
    conn: sqlite3.Connection,
    row: sqlite3.Row,
    review_repository: ReviewRepository | None = None,
) -> str:
