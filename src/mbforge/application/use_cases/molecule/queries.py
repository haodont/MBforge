...
from mbforge.application.dto.molecule import MoleculeListRequest, MoleculeListResponse
from mbforge.application.pipeline.persist.molecules import filter_persistable_candidates
from mbforge.application.ports import get_database
from mbforge.application.use_cases.documents.pdf_layout import load_document_bboxes
...
def search_molecules(
    ...
    """
    with get_database(library_root).mol_conn() as conn:
        if mode == "text":
...
def molecules_by_location(
    ...
    :func:`mbforge.application.use_cases.documents.pdf_layout.load_document_bboxes`; those
...
def delete_molecules(root: str, mol_ids: list[str]) -> int:
    ...
    db = get_database(root)
    with db.mol_conn() as conn:
        return db.delete_molecule_records(conn, mol_ids)
