...
from mbforge.application.dto.review import (
...
def insert_review_item(
    ...
    *,
    ...
    """Insert one review item, retaining the connection-level compatibility API."""
    if review_repository is not None:
        return review_repository.insert_review_item(
            conn,
            kind=kind,
            doc_id=doc_id,
            page=page,
...
def _get_repositories(library_root: str | None):
...
def _get_db(library_root: str | None):
    return _get_repositories(library_root).database
