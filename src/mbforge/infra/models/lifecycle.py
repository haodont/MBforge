...
inference. HTTP endpoints live in ``routers/system/models.py``; the
render endpoint stays there until the M2 chem 汇合点
(TODO/services-layer-plan.md A8).
...
def clear(model_id: str | None = None) -> dict[str, Any]:
    ...
    if model_id in (None, "moldet"):
        from ...backends import moldet_v2_ft
...
def run_test(model_id: str, subpath: str | None = None) -> dict[str, Any]:
    ...
    try:
        if model_id == "moldet":
            from ...backends.moldet_v2_ft import MolDetv2Detector
            ...
            from ...backends.molparser import load as load_molparser
