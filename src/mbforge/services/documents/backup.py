...
def _rows_by_doc(conn: sqlite3.Connection, table: str, doc_id: str) -> list[dict[str, Any]]:
...
def _rows_in(conn: sqlite3.Connection, table: str, column: str, ids: list[str]) -> list[dict[str, Any]]:
...
def _snapshot_doc_db(conn: sqlite3.Connection, doc_id: str) -> dict[str, list[dict[str, Any]]]:
    ...
    candidate_ids = [
        row["candidate_id"]
        for row in snapshot.get("markush_review_candidates", [])
    ]
    if candidate_ids:
...
def _rows_by_owner(conn: sqlite3.Connection, table: str, doc_id: str) -> list[dict[str, Any]]:
