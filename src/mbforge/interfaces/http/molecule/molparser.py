...
from mbforge.application.pipeline.detection.recognition import recognize_molecule
from mbforge.application.ports import get_runtime
from mbforge.foundation.errors import ModelNotAvailableError, ValidationError
from mbforge.foundation.files import decode_base64_to_tempfile
from mbforge.foundation.logger import get_logger

logger = get_logger("mbforge.interfaces.http.molparser")
...
async def molparser_predict(request: Request) -> dict[str, Any]:
    ...
    runtime = get_runtime()
    await loop.run_in_executor(None, runtime.molparser.load)
    result = await loop.run_in_executor(None, lambda: runtime.molparser.predict(image))
    ...
    runtime.model_status.ensure("molparser", "ready")
    return {
...
async def molparser_recognize(request: Request) -> dict[str, Any]:
    ...
    get_runtime().model_status.ensure("molparser", "ready")
    return {
        "esmiles": result.esmiles,
        "smiles": result.smiles,
        "coref": result.coref,
