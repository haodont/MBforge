...
async def resource_download(body: ResourceIdRequest) -> dict:
    ...
    result = await asyncio.to_thread(
        get_runtime().resource_manager.ensure, body.resource_id
    )
...
async def resources_check() -> dict:
    report = await asyncio.to_thread(get_runtime().resource_manager.check_all)
    return {
        "resources": [
            {
                "id": r.id,
...
def _model_subfiles(resource_id: str) -> list[dict]:
    ...
    resource_manager = get_runtime().resource_manager
...
async def resources_catalog() -> dict:
    ...
    return {
        "resources": [
            ...
            for info in resource_manager.catalog.values()
            if info.type.value == "model"
        ]
    }
