...
→ **本模块已用 `column_sort` 启发式补上**（`layout/reading_order.py`，逐行移植自官方
Hiro-Smart-Doc，已验证与官方实现输出完全一致）。检测器用 `provides_reading_order`
自描述是否自带顺序：V3 为 `True`（保持原样——重排只会破坏它），Hiro 为 `False`（触发后处理）。
→ **组装层不必再恢复顺序**，可直接消费 `regions` 的下标与 `reading_order` 字段。局限见 §9 风险 3。
...
> **阅读顺序的归属**：原先认为这是本层责任，**现已上移到版面识别层**
> （`layout/reading_order.py`，见 §4.2 与 §9 风险 3）。
> 因此本层不必再恢复顺序，直接消费 `regions` 的下标即可。
...
3. ~~**Hiro 不输出阅读顺序**~~ → **已解决（2026-09-18）**。
   已在**版面识别层**实现：`layout/reading_order.py` **逐行移植**官方 Hiro-Smart-Doc 的
   `column_sort` + `determine_columns`，并用官方实现当 oracle 验证等价
   （7 手工用例 + 3000 随机模糊 + 768 真实页面，**输出完全一致**）。
   实测 256 页：Hiro 原始序 y 回跳 **1425 → 86**、栏切换 **571 → 314**，
   **追平并略优于 V3 自带顺序**（99 / 354）。
   ⚠️ **已知局限**：约 22% 页面仍有栏内交错（V3 为 27%），根因是 `determine_columns`
   的**栏数判定**——实测 `US20220401435A1_p0047` 实为 2 栏却被判 3 栏，右栏框被拆进第 2/3 栏，
   左栏框的"跨栏回灌"又把右栏框一并吸进左栏。阈值调优留作后续（`layout/README.md` §12.10）。
